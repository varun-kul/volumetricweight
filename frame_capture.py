...
class Camera_Thread:
    ...
    camera_width2 = 640
    camera_height2 = 480
    camera_frame_rate2 = 30
    camera_fourcc2 = cv2.VideoWriter_fourcc(*"MJPG")
    ...
    def start(self):
        ...
        #self.camera = cv2.VideoCapture(self.camera_source1)
        ...
        #Camera 2 setup 
        self.camera2 = cv2.VideoCapture(self.camera_source1)  # Set the source for the second camera
        self.camera2.set(3, self.camera_width2)  # Set the width for the second camera
        self.camera2.set(4, self.camera_height2)  # Set the height for the second camera
        self.camera2.set(5, self.camera_frame_rate2)  # Set the frame rate for the second camera
        self.camera2.set(6, self.camera_fourcc2)  # Set the fourcc for the second camera
        time.sleep(self.camera_init2)

        # Camera image vars for the second camera
        self.camera_width2 = int(self.camera2.get(3))
        self.camera_height2 = int(self.camera2.get(4))
        self.camera_frame_rate2 = int(self.camera2.get(5))
        self.camera_mode2 = int(self.camera2.get(6))
        self.camera_area2 = self.camera_width2 * self.camera_height2

        # Buffer for the second camera
        self.buffer2 = queue.Queue(1)


        # Start the thread for the second camera
        self.frame_grab_run2 = True
    ...
    def loop(self):
        ...
        #frameN = self.black_frame
        if not self.buffer.full():
            self.buffer.put(frame,False)
            #self.buffer.put(frameN,False)
        ...
        while 1:
            ...
            if self.buffer_all:
                ...
                if self.buffer2.full():
                    time.sleep(1/self.camera_frame_rate2)

                # or load buffer with next frame
                else:
                    
                    grabbed2,frameN = self.camera2.read()
            ...
            # false buffered mode (for camera, loss allowed)
                ...
                grabbed2,frameN = self.camera2.read()
                #grabbed1,frameN = self.camera2.read()
                if not grabbed2:
                    break
            ...
            # update frame read rate
            if fc1 >= 10:
                self.current_frame_rate2 = round(fc1/(time.time()-t11),2)
                fc1 = 0
                t11 = time.time()
    ...
    def next1(self,black=True,wait=0):
        ...
        # get from buffer (fail if empty)
        try:
            frameN = self.buffer2.get(timeout=wait)
            self.frames_returned += 1
        except queue.Empty:
            #print('Queue Empty!')
            #print(traceback.format_exc())
