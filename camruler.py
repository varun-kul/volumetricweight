        ...
        for line in f:
            ...
            if line and line[0] != '#' and (',' in line or '=' in line):
                if ',' in line:
                    ...
                    continue                        
                if item in 'camera_id camera_id1 camera_width camera_height camera_frame_rate camera_fourcc auto_percent auto_threshold auto_blur norm_alpha norm_beta'.split():
                    try:
                        exec(f'{item}={value}')
...
if len(sys.argv) > 1:
    ...
    camera_id1 = sys.argv[1]
    if camera_id1.isdigit():
        camera_id1 = int(camera_id1)

# camera thread setup
camera = frame_capture.Camera_Thread()
camera.camera_source = camera_id # SET THE CORRECT CAMERA NUMBER
#camera.camera_source1 = camera_id1
...
camera1 = frame_capture.Camera_Thread()
#camera.camera_source = camera_id # SET THE CORRECT CAMERA NUMBER
camera1.camera_source1 = camera_id1
camera1.camera_width  = camera_width
camera1.camera_height = camera_height
camera1.camera_frame_rate = camera_frame_rate
camera1.camera_fourcc = camera_fourcc
...
print('CAMERA1:',[camera.camera_source,width,height,area,frate])
print('CAMERA:2',[camera.camera_source1,width,height,area,frate])
...
while 1:
    ...
    cv2.normalize(frameN,frameN,norm_alpha,norm_beta,cv2.NORM_MINMAX)

    # rotate 180
    if key_flags['rotate']:
        frame0 = cv2.rotate(frame0,cv2.ROTATE_180)
        frameN = cv2.rotate(frameN,cv2.ROTATE_180)
    ...
    text.append(f'CAMERA: {camera_id1} {width}x{height} {fps}FPS')
    ...
    if key_flags['config']:
        ...
        draw.crosshairs(frameN,5,weight=2,color='red',invert=True)
        ...
        draw.line(frameN,cx,cy, cx+cx, cy+cy,weight=1,color='red')
        draw.line(frameN,cx,cy, cx+cy, cy-cx,weight=1,color='red')
        draw.line(frameN,cx,cy,-cx+cx,-cy+cy,weight=1,color='red')
        draw.line(frameN,cx,cy, cx-cy, cy+cx,weight=1,color='red')
        ...
        draw.line(frameN,mx,my,mx+dm,my+(dm*( cy/cx)),weight=1,color='green')
        draw.line(frameN,mx,my,mx-dm,my-(dm*( cy/cx)),weight=1,color='green')
        draw.line(frameN,mx,my,mx+dm,my+(dm*(-cx/cy)),weight=1,color='green')
        draw.line(frameN,mx,my,mx-dm,my-(dm*(-cx/cy)),weight=1,color='green')
        ...
        draw.add_text(frameN,caltext,(cx)+100,(cy)+30,color='red')
    ...
    #-------------------------------
        ...
        frame11 = cv2.cvtColor(frameN,cv2.COLOR_BGR2GRAY)

        # blur frame
        frame1 = cv2.GaussianBlur(frame1,(auto_blur,auto_blur),0)
        frame11 = cv2.GaussianBlur(frame11,(auto_blur,auto_blur),0)

        # threshold frame n out of 255 (85 = 33%)
        frame1 = cv2.threshold(frame1,auto_threshold,255,cv2.THRESH_BINARY)[1]
        frame11 = cv2.threshold(frame11,auto_threshold,255,cv2.THRESH_BINARY)[1]
        ...
        contours1,nada1 = cv2.findContours(frame11,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
        
        # small crosshairs (after getting frame1)
        draw.crosshairs(frame0,5,weight=2,color='green') 
        draw.crosshairs(frameN,5,weight=2,color='green')    
        ...
        for c in contours:
            ...
            #draw.rect(frameN,x1,y1,x2,y2,weight=2,color='red')
            ...
            # draw.add_text(frameN,f'{xlen:.2f}',x1-((x1-x2)/2),min(y1,y2)-8,center=True,color='red')
            # draw.add_text(frameN,f'Area: {carea:.2f}',x3,y2+8,center=True,top=True,color='red')
            # if alen:
            #     draw.add_text(frameN,f'Avg: {alen:.2f}',x3,y2+34,center=True,top=True,color='green')
            # if x1 < width-x2:
            #     draw.add_text(frameN,f'{ylen:.2f}',x2+4,(y1+y2)/2,middle=True,color='red')
            # else:
            #     draw.add_text(frameN,f'{ylen:.2f}',x1-4,(y1+y2)/2,middle=True,right=True,color='red')
    ...
    #-------------------------------
        ...
        draw.vline(frameN,mouse_raw[0],weight=1,color='green')
        draw.hline(frameN,mouse_raw[1],weight=1,color='green')
        ...
        if mouse_mark:
            ...
            draw.rect(frameN,x1,y1,x2,y2,weight=weight,color='red')
            draw.line(frameN,x1,y1,x2,y2,weight=weight,color='green')
            ...
            draw.add_text(frameN,f'{xlen:.2f}',x1-((x1-x2)/2),min(y1,y2)-8,center=True,color='red')
            draw.add_text(frameN,f'Area: {carea:.2f}',x3,y3+8,center=True,top=True,color='red')
            if alen:
                draw.add_text(frameN,f'Avg: {alen:.2f}',x3,y3+34,center=True,top=True,color='green')           
            if x2 <= x1:
                draw.add_text(frameN,f'{ylen:.2f}',x1+4,(y1+y2)/2,middle=True,color='red')
                draw.add_text(frameN,f'{llen:.2f}',x2-4,y2-4,right=True,color='green')
            else:
                draw.add_text(frameN,f'{ylen:.2f}',x1-4,(y1+y2)/2,middle=True,right=True,color='red')
                draw.add_text(frameN,f'{llen:.2f}',x2+8,y2-4,color='green')
    ...
    draw.add_text_top_left(frameN,text)

    # display
    cv2.imshow('cam1',frame0)
    cv2.imshow('cam2',frameN)
